...
if __name__ == "__main__":
    ...
    def initialize_model(model, max_tokens, temperature, freq_penalty, rep_penalty, max_conlen, top_k, top_p):
        global rkllm_model
        try:
        ...
        print(init_msg)
        sys.stdout.flush()
        rkllm_model = RKLLMLoaderClass(model=model, maxnewtok=max_tokens, temp=temperature, freqpen=freq_penalty, reppen=rep_penalty, maxconlen=max_conlen, topk=top_k, topp=top_p )
        model_init = f"RKLLM Model: {rkllm_model.model_name} has been initialized successfully！\nIn case of errors, the container will restart automatically. Wait some seconds and reload this page to start again."
        print(model_init)
        complete_init = "====================================================================================="
        print(complete_init)
        print()
        ...
        sys.stdout.flush()
        #return output
        return rkllm_model.rkllm_param.max_new_tokens, rkllm_model.rkllm_param.temperature, rkllm_model.rkllm_param.frequency_penalty, rkllm_model.rkllm_param.repeat_penalty, rkllm_model.rkllm_param.max_context_len, rkllm_model.rkllm_param.top_k, rkllm_model.rkllm_param.top_p,  output
 ...
 # INTERFACE
    ...
    with gr.Blocks(fill_width=True, theme=theme, title="AI Chatbot") as chatRKLLM:
        ...
        with gr.Row(height=105, variant='panel'):
            available_models = available_models()
            model_dropdown = gr.Dropdown(choices=available_models, label="Select the Model to load", value="None", allow_custom_value="True")
            statusBox = gr.Textbox(lines=3, container=False, show_label=False)

        with gr.Row():
            # HYPERPARAMETERS
            with gr.Column(scale=1):
                max_conlen = gr.Slider(minimum=1024, maximum=70000, value=0, step=1, label="Max Context Lenght")
                max_tokens = gr.Slider(minimum=1024, maximum=8192, value=0, step=1, label="Max New Tokens")
                ...
                rldbtn = gr.Button("Reload the Model with the parameters above", variant="primary")
                rldbtn.click(fn=initialize_model, inputs=[model_dropdown, max_tokens, temperature, freq_penalty, rep_penalty, max_conlen, top_k, top_p], outputs=[max_tokens, temperature, freq_penalty, rep_penalty, max_conlen, top_k, top_p, statusBox])

                model_dropdown.change(fn=initialize_model, inputs=model_dropdown, outputs=[max_tokens, temperature, freq_penalty, rep_penalty, max_conlen, top_k, top_p, statusBox])
            ...
            with gr.Column(scale=3):
              with gr.Tabs():
               with gr.TabItem("Live Stream Chat (Seamless, no Statistics)"):
                ...
                txt2txt = gr.ChatInterface(fn=get_RKLLM_output, type="messages", stop_btn=False)
                txt2txt.chatbot.height = "60vh"
                txt2txt.chatbot.resizable = True

               with gr.TabItem("Chat with Statistics (no Streaming)"):

                chatbot = gr.Chatbot(type="messages", show_copy_button=True, avatar_images=['user.png', 'bot.png'], height=650, layout='bubble', resizeable=True)
                ...
                def chat(message, history, temperature, freq_penalty, rep_penalty, max_tokens, max_conlen, top_k, top_p):
                    """
...
'''
                ...
                msg.submit(chat, [msg, chatbot, temperature, freq_penalty, rep_penalty, max_tokens, max_conlen, top_k, top_p], [chatbot, msg, ttft_seconds, inference_time, speed, tokens_stats])
